...
* simplified RAG setup: Project text information is contanated and embedded with ada-2. Vector search was not very reliable, and the following LLM  evaluation was too slow/costly per query.
...
* ProjectPosition, string, nullable. May be mapped to the predefined StandardPosition
...
* "Leadership experince including coordinating >5 international teams"
...
1. **Load projects** into SQLite `projects` table (your data import)
...
3. **Generate synthetic queries** using LLM:
   ```bash
   python generate_queries.py          # generates 2 queries per project
